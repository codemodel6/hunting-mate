...
import type { ChatDetail as Chat, ChatMessage as Message } from "@/entities/chat/model/chat";
import { getProfile } from "@/entities/profile/api/profile-api";
import { getUserId, isAuthenticated } from "@/entities/session/api/session-api";
...
export default function Page({
  ...
  const canAccept =
    chat?.matchStatus === "requested" && chat.matchRequester !== currentUserId;
        ...
        <aside className="space-y-6">
          <div className="rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-rose-100">
            ...
            <h1 className="mt-4 text-3xl font-semibold text-slate-900">
              {chat?.otherUser.name ?? "대화 상대"}
            </h1>
            <p className="mt-2 text-sm text-slate-500">
              상태: {chat?.matchStatus ?? "로딩 중"}
            </p>
            ...
            {chat?.otherUser.photos?.[0] && (
              <img
                ...
                className="mt-6 aspect-[4/3] w-full rounded-[1.5rem] object-cover"
              />
            )}
            ...
            <div className="mt-4 grid gap-3">
              {canRequest && (
                <button
                  type="button"
                  onClick={() =>
                    void runAction(() => requestMatch(chatId), "매칭 요청을 보냈습니다.")
                  }
              ...
              {canAccept && (
                <button
                  type="button"
                  onClick={() =>
                    void runAction(() => acceptMatch(chatId), "매칭을 수락했습니다.")
                  }
                  className="rounded-2xl bg-rose-500 px-4 py-3 text-sm font-medium text-white transition hover:bg-rose-600"
                >
                  매칭 수락
